...
FROM ORDERS JOIN [ORDER DETAILS] AS OD ON ORDERS.ORDERID = OD.ORDERID JOIN PRODUCTS ON OD.PRODUCTID = PRODUCTS.PRODUCTID JOIN CATEGORIES ON PRODUCTS.CATEGORYID = CATEGORIES.CATEGORYID
WHERE EXISTS (SELECT CUSTOMERID FROM CUSTOMERS WHERE REGION = 'WA')	;	
